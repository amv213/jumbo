...
**Note:** It's more than likely that the user is not connecting to the database from the same computer that is running the server
...
Jumbo automatically extracts our database connection settings from a jumbo.env file with the following structure:
...
As an example, the jumbo.env file that the user from the tutorial above should be creating should look like this:
...
You should now have everything configured as it should. Remember to create a jumbo.env file in the root directory of your
